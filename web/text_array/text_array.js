...
class TextArrayWidget {
    ...
    #createTabElement(tabIndex, {onDelete, onSelect, parent=null}){
        ...
        LocodeUtils.createElement("span", {
            ...
            content:    '<svg xmlns="http://www.w3.org/2000/svg" width="16px" height="16px" viewBox="0 0 24 24" fill="currentColor"><path d="M0 0h24v24H0z" fill="none"/><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>',
            listeners: {
                // Обработчик удаления
                "click": (e) => {
                    e.stopPropagation();
    ...
    #updateNodeValue() {
        ...
        this.node.properties.lo_text_array = Array.isArray(this.texts) ? [...this.texts] : [String(this.texts ?? "")];
    ...
    setValue(value) {
        if (Array.isArray(value)) {
            this.texts = value.length > 0 ? [...value] : [""];
            // Сброс активной вкладки в допустимые границы
            this.activeTab = Math.min(this.activeTab, this.texts.length - 1);
        }
        // Обновляем состояние
        this.#buildTabs();
        this.#setState();
...
//   только для узла с именем "LoTextArray".
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        if (nodeData.name === "LoTextArray") {
            ...
            nodeType.prototype.onNodeCreated = function() {
                ...
                console.debug("onNodeCreated", this, arguments, ret);
                // создаём и сохраняем ссылку на виджет
                this._lo_text_array_widget = new TextArrayWidget(this, "text_array", {}, app);
                // если есть сохранённые значения в properties — загрузим их
                try {
                    const saved = this?.properties?.lo_text_array;
                    if (Array.isArray(saved) && saved.length) {
                        this._lo_text_array_widget.setValue(saved);
                    }
                } catch (e) {
                    console.warn("LoTextArray restore on create failed", e);
                }
                return ret;
            };
            ...
            nodeType.prototype.onSerialize = function(o) {
                ...
                try {
                    ...
                    const value = this?._lo_text_array_widget?.getValue?.();
                    o.properties.lo_text_array = Array.isArray(value) ? [...value] : (value ? [String(value)] : [""]);
                } catch (e) {
            ...
            nodeType.prototype.onConfigure = function(o) {
                ...
                try {
                    const saved = o?.properties?.lo_text_array;
                    if (Array.isArray(saved) && this?._lo_text_array_widget) {
                        this._lo_text_array_widget.setValue(saved);
                    }
                } catch (e) {
                    console.warn("LoTextArray onConfigure warning", e);
                }
                return ret;
            };
        }
